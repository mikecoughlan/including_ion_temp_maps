...
TARGET = 'rsd'
VERSION = 'swmag_v6-oversampling_residual'
...
class ResidualSWMAGModel(nn.Module):
	'''
	Class to create the model for the TWINS data.
	'''

	def __init__(self, inital_filters:int, input_shape:tuple):
		super(ResidualSWMAGModel, self).__init__()

		self.input_shape = input_shape
		self.inital_filters = inital_filters

		self.conv = nn.Conv2d(in_channels=1, out_channels=inital_filters, kernel_size=(2,2), stride=(1,1), padding='same', bias=False)
		self.batchnorm = nn.BatchNorm2d(inital_filters)
		self.relu = nn.ReLU()

		self.block_0 = nn.Sequential(
			ResidualBlock(in_channels=inital_filters, out_channels=inital_filters, kernel_size=(2,2), stride=(1,1)),
			ResidualBlock(in_channels=inital_filters, out_channels=inital_filters, kernel_size=(2,2), stride=(1,1)),
			ResidualBlock(in_channels=inital_filters, out_channels=inital_filters, kernel_size=(2,2), stride=(1,1)),
		...
		self.block_1 = nn.Sequential(
			ResidualBlock(in_channels=inital_filters, out_channels=inital_filters*2, kernel_size=(2,2), stride=(1,1)),
			ResidualBlock(in_channels=inital_filters*2, out_channels=inital_filters*2, kernel_size=(2,2), stride=(1,1)),
			ResidualBlock(in_channels=inital_filters*2, out_channels=inital_filters*2, kernel_size=(2,2), stride=(1,1)),
		)

		self.block_2 = nn.Sequential(
			ResidualBlock(in_channels=inital_filters*2, out_channels=inital_filters*4, kernel_size=(2,2), stride=(1,1)),
			ResidualBlock(in_channels=inital_filters*4, out_channels=inital_filters*4, kernel_size=(2,2), stride=(1,1)),
			ResidualBlock(in_channels=inital_filters*4, out_channels=inital_filters*4, kernel_size=(2,2), stride=(1,1)),
		)

		self.fc_block = nn.Sequential(

			nn.Linear(in_features=(int((input_shape[0]/2)*(input_shape[1]/2)*(inital_filters*4))),  out_features=inital_filters*4),
			nn.ReLU(),
			nn.Dropout(0.2),
	...
	def forward(self, twins):
		...
		twins = torch.reshape(twins, (-1, int((self.input_shape[0]/2)*(self.input_shape[1]/2)*(self.inital_filters*4))))
	...
	if not finished_training:
		...
		while current_epoch < num_epochs:
			...
			if (early_stopping(train_loss=loss, val_loss=val_loss, model=model, optimizer=optimizer, epoch=current_epoch)) or (current_epoch == num_epochs-1):
				...
				model = None
				model = ResidualSWMAGModel(inital_filters=32, input_shape=(30, 14))
...
def main():
	...
	model = ResidualSWMAGModel(inital_filters=32, input_shape=(train_size[1], train_size[2]))
